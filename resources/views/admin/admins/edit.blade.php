<x-admin-layout>
    ...
    <div class="mb-8 flex items-center justify-between gap-4">
        <div>
            <h2 class="text-3xl font-black text-slate-900 mb-1">Edit Administrator</h2>
            <p class="text-slate-500 text-sm font-medium">Update settings and permissions for: {{ $admin->name }}.</p>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-slate-50 pt-6">
                ...
                <div>
                    ...
                    <select name="role" id="role" required
                            class="w-full rounded-xl border-slate-200 focus:border-slate-800 focus:ring focus:ring-slate-200 text-slate-800 font-medium">
                        <option value="Admin" {{ $admin->role === 'Admin' ? 'selected' : '' }}>Standard Admin</option>
                        <option value="Super Admin" {{ $admin->role === 'Super Admin' ? 'selected' : '' }}>Super Admin (All Permissions)</option>
                    </select>
                    @error('role')
                        <span class="text-xs text-red-600 mt-1 block">{{ $message }}</span>
                    @enderror
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                        ...
                        <option value="inactive" {{ $admin->status === 'inactive' ? 'selected' : '' }}>Inactive (Disabled)</option>
                    </select>
                    @error('status')
                        <span class="text-xs text-red-600 mt-1 block">{{ $message }}</span>
                    @enderror
                    ...
                    @foreach($permissions as $permission)
                        <label class="flex items-center gap-3 p-4 border border-slate-100 rounded-xl hover:bg-slate-50 cursor-pointer transition">
                            <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" 
                                   ...
                                   class="rounded text-slate-800 focus:ring-slate-200 h-4 w-4">
                            <div>
