<x-admin-layout>
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h2 class="text-3xl font-black text-slate-900 mb-1">Customer Enquiries</h2>
            <p class="text-slate-500 text-sm font-medium">Review customer general requests and send responses.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm mb-8 max-w-sm">
        <form method="GET" action="{{ route('admin.enquiries.index') }}" class="flex items-end gap-4">
            <div class="flex-1">
                <label for="status" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Filter Status</label>
            ...
            <table class="w-full text-left text-sm text-slate-500">
                <thead>
                    <tr class="border-b border-slate-100 text-xs font-bold uppercase text-slate-400 bg-slate-50/50">
                        <th class="py-4 px-6 rounded-l-xl">Customer</th>
                    ...
                    @forelse($enquiries as $enquiry)
                        <tr class="hover:bg-slate-50/50 transition">
                            ...
                            <td class="py-4 px-6 font-medium text-slate-600">
                                {{ $enquiry->service->name ?? 'Deleted Service' }}
                            </td>
                            ...
                            <td class="py-4 px-6 text-right">
                                <a href="{{ route('admin.enquiries.show', $enquiry->id) }}" class="text-slate-400 hover:text-teal-600 transition font-bold text-xs">
                                    Reply <i class="fa-solid fa-arrow-right text-[10px] ml-1"></i>
                                </a>
                            </td>
                        </tr>
                    @empty
